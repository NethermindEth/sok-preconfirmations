...
\chm{I don't really understand how this affects preconfer schedule. It seems the schedule is just whatever it is and this is a remark about the credibility of preconfs given the current schedule.}

\paragraph{ How the type of the blockchain affects preconfer schedule}
\begin{itemize}
    \item \textbf{L1 and based L2 \chm{proposer preconfs}}: Proposers are determined via the L1 proposer election mechanism. L1 preconfer registries -- while not responsible for electing L1 proposers -- can still determine the preconfer schedule, e.g. through EIP-7917~\cite{EIP7917} \todoks{examples consistency}. The preconfer schedule can be determined by finding the intersection of the registered preconfer set and the proposer lookahead, and taking into account whether the preconf is inclusion or execution.
    \item \textbf{Non-based L2 \chm{proposer preconfs}}: Within non-based L2s, there is a large proposer-preconfer election design space. The preconfer registry can either read from the outputs of a standalone proposer election mechanism as in L1 and based L2 preconfs, or be integrated into the proposer election mechanism ~\cite{, Optimism, Arbitrum, ZKsync, Starknet}. \tododk{Dimitris, we need a reference for non-based L2s}
    \item \chm{\textbf{Block builder preconfs}. When block builders issue preconfs, the preconfer schedule cannot be predetermined but rather depends on the outcome of the block builder auction. In this case, all registered block builders can issue conditional preconfs, conditioned on the corresponding block builder winning the block builder auction. Which of the builders wins the auction and thus is responsible for fulfilling the preconfs is then determined after the preconfing has taken place. It therefore makes sense for the users to source preconfs from all the registered block builders~\cite{W:LeaderlessandLeader-BasedPreconfirmations}.}
\end{itemize}
...
In this section, we describe how the mev-commit protocol~\cite{W:MevCommitWhitepaper,W:Documentation-Understandingmev-commit} issues preconfirmations, or \emph{commitments} in the protocol's terminology. While we follow the general structure introduced in Section~\ref{sec:preconf_pipeline}, it is important to note that sometimes mev-commit slightly departs from that framework. 

The central idea of mev-commit is to enable L1 block builders to provide users with enforceable, yet conditional, assurances about transaction inclusion or execution before the corresponding L1 block is built. To achieve this, the protocol maintains a dedicated mev-commit blockchain with fast block times. When a user requests a preconf by submitting a bid, a block builder may issue a conditional preconf where the condition is they build the specified L1 block. To issue such a preconf, the builder records a cryptographic commitment on the mev-commit chain. These commitments hide all details of the bid but can later be opened to allow public verification and settlement. The commitments are supposed to be opened by the block builder after building the corresponding L1 block. If the builder fails to open, e.g., because the promised conditions were not met, the user can instead open the commitment to prove non-fulfillment. In this way, mev-commit enforces accountability for preconfirmations without sacrificing confidentiality at the bidding stage.

Settlement relies on an oracle that monitors the L1 chain. The oracle looks at opened commitments and checks whether the conditions for fulfillment are met. If so, the builder receives the preconf tip specified in the bid. If not, the builder is penalized by transferring the compensation amount specified in the bid to the user.

Next, we describe the six steps structure from Section~\ref{sec:preconf_pipeline} for mev-commit.

\begin{description}
    \item[Registration:] Block builders enroll in the protocol through the \emph{Provider Registry}, a smart contract deployed on the mev-commit chain. To enroll, builders must deposit a minimum stake to ensure they have sufficient funds to cover potential penalties.
    
    \item[Election:] All registered block builders can issue conditional preconfs at any time, conditioned on building the L1 block at the specified height. As such, there is no election process to determine which builder can issue preconfs for a given block. Instead, the MEV-Boost auction determines which builder will ultimately build the L1 block and thus be responsible for fulfilling the preconfs they issued.
    
    \item[Request:] Users may request preconfs by sending encrypted bids to registered block builders. The bid specifies the transaction bundle to be preconfed, the block height for which the preconf is requested, the preconf tip (the fee offered to the builder for fulfilling the preconf), and the compensation amount (the penalty the builder must pay if they fail to fulfill the preconf). The bid further specifies whether the request is for a simple inclusion preconf or a preconf that guarantees execution of the transactions in the bundle without reversion. Advanced bid options including positional constraints dictating the bounds on the position of the transactions within the block are also supported. Finally, the bid contains the current timestamp and an expiration time after which the bid is no longer valid.
    
    \item[Response:] Upon receiving a preconf request, a block builder may ignore it or issue a preconf by publishing a cryptographic commitment on the mev-commit chain. This also provides an on-chain timestamp of when the commitment was issued, which is used to compute the bid decay (to solve the fair-exchange problem, see below).
    
    \item[Fulfillment:] When a builder~$B$ produces an L1 block at some height, all preconfs issued by~$B$ for that height become enforceable. If $B$ fails to honor them, penalties apply.
...
To address the \textbf{fair-exchange problem}, bid amounts decay over time, computed as linear function decreasing from the initial bid amount to zero at the expiration time set in the bid. The decayed amount is computed using the timestamp of when the commitment was issued as recorded on the mev-commit chain. This incentivices builders to issue preconfs in a timely manner after receiving the bid.
