...
\newcommand{\kt}[1]{\textcolor{purple}{\textbf{Katerina:} #1}}
...
\newcommand{\todokt}[1]{\todo[color=purple!40]{\textbf{Katerina:} #1}}
...
\label{EconomicViabilityOfPreconfs}
    ...
    \subsection{Preconfer caveats} \label{preconfer_caveats}
        ...
        % Additionally, as seen in \ref{preconfer_faults_and_punishing_conditions}, a chained preconfs system was introduced to prevent accidental liveness slashing~\cite{W:AvoidingAccidentalLivenessFaultsforBasedPreconfs}.
    ...
    \subsection{User caveats}
        ...
        Preconfers are not the only entity that will need to become more sophisticated. Preconfs also entail some additional sophistication for users~\cite{W:AnalysingExpectedProposerRevenuefromPreconfirmations}. Users requesting a preconf must calculate an appropriate preconf tip, compose a valid preconf request (as seen in Section~\ref{preconf_request}) and identify the right preconfer(s) or endpoint to send requests to~\cite{W:AnalyzingBFTProposer-PromisedPreconfirmations}. However, wallets might abstract away this complexity for users by creating preconf requests with default parameters, similarly to how they automatically set the gas limit, and give advanced users the option to manually set the preconf request parameters~\cite{W:PreconfirmationsforVanillaBasedRollups}. 
        \begin{considerationbox}{Requirement for user sophistication}
            \consideration{Added complexity for getting transactions preconfed.}
            \mitigation{Have wallets abstract away complexity.}
        \end{considerationbox}
        A further consideration for users is the implication of preconfs on gas pricing and fees. As explained in Section~\ref{EconomicViabilityOfPreconfs}, the viability of preconfing depends on sufficient economic incentives. For this reason, preconfed transactions are expected to be more costly than regular transactions, with preconf tips compensating preconfers for their service. Furthermore, preconfs provide users with better inclusion and execution commitments compared to regular transactions. In the long run, this could result in an increased demand for blockspace, which would affect gas pricing for all users. \todoqb{im not convinced by the gas pricing claim but I left it in for now} There are no direct mitigations for users regarding the additional cost of preconf tips. However, if there is sufficient demand for preconfs, preconfers may require lower tips for this service to remain economically viable.
        % A further consideration for users is the implications of preconfs on transaction fees. As explained in Section \dk{add reference to the Economics section}, the viability of preconfing depends on sufficient economic incentives. For this reason, preconfed transactions are expected to be more costly than regular transactions, with preconf tips compensating preconfers for their service. \qb{not convinced by the gas pricing argument}. There is no direct mitigation for users in this regard. However, if there is sufficient demand for preconfs, preconfers may require lower tips for this service to remain economically viable. \qb{flawed}
